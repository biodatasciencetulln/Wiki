...
- Install VMware Fusion according to [knowledge.broadcom.com](https://knowledge.broadcom.com/external/article/368667/download-and-license-information-for-vmw.html). The download requires creating a [Broadcom account](https://www.reddit.com/r/vmware/comments/1kolpgb/how_to_download_vmware_fusion_free/).
...
For Ubuntu releases prior to 24.10, Canonical doesn't provide an Ubuntu Desktop ISO for platforms other than [x86_64](https://www.techtarget.com/whatis/definition/x86-64) and Raspberry Pi. So, there is no installer for Ubuntu
Desktop on ARM. You can work around this by installing **Ubuntu Server** for ARM and then converting it to Ubuntu Desktop by installing additional software, the **desktop environment**. This is the method of choice for both UTM and VMware Fusion. You will need the ISO file of the latest [Ubuntu Server](https://ubuntu.com/download/server/arm) LTS release, currently 24.04.
...
- For Ubuntu 24.04, you'll install Ubuntu Server ([ubuntu.com](https://ubuntu.com/download/server/arm)) and add the GUI ("graphical user interface", i.e. the desktop environment) in the next step, rather than directly installing Ubuntu Desktop. If you prefer Ubuntu 25.04, you can use the Ubuntu Desktop ISO for [ARM](https://ubuntu.com/download/desktop) instead; in this case, the desktop environment is already preinstalled.
...
- Recommended YouTube walkthrough: "How to install Ubuntu on Macbook M1 or M2 Using VMWare Fusion" by Murphy Tsai, 05/2023 ([YouTube](https://www.youtube.com/watch?v=4dFy-4pw8NA))
  - At 6:30, the presenter opens the "Fusion 13 for Apple Silicon **Companion Guide**" and enters commands from there. This is an unofficial user-contributed document that provides tips and techniques for setting up and running virtual machines with VMware Fusion 13 on Apple Silicon Macs. You should use the most up-to-date version of the [companion guide](https://community.broadcom.com/vmware-cloud-foundation/viewdocument/the-unofficial-fusion-for-apple-sil) (v32 at the time of writing), and use the commands for your specific distribution version. You need only the "Ubuntu" section.
